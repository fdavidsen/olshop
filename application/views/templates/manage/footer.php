            	</div>
          </div>
        </div>
      </div>

      <!-- Menu Toggle Script -->
      <script type="text/javascript">
        $('#great-container').removeClass('container mt-3 mb-5');
      </script>
    </div>



    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.0/js/bootstrap.min.js" integrity="sha384-OgVRvuATP1z7JjHLkuOU7Xw704+h835Lr+6QL9UvYjZE3Ipu6Tp75j7Bh/kR0JKI" crossorigin="anonymous"></script>
    <!-- Base URL -->
    <script type="text/javascript">const baseURL = $('#base-url').html();</script>
    <!-- My JavaScript -->
    <script type="text/javascript" src="<?= base_url(); ?>public/js/functions.js"></script>
    <script type="text/javascript" src="<?= base_url(); ?>public/js/sidebar.js"></script>
    <script type="text/javascript" src="<?= base_url(); ?>public/js/script.js"></script>
    <script type="text/javascript" src="<?= base_url(); ?>public/js/search.js"></script>
  </body>
</html>